...
class Player {
    public:
        ...
        void loadSprite(std::string filePath) {
            ...
            // only really works well if the entity is a perfect square
            this->bodyCenter.setRadius(this->size.x / 2);
        }
        ...
        void render() {
            ...
            this->bodyCenter.setPosition(this->playerSprite.getPosition().x - this->size.x, this->playerSprite.getPosition().y - this->size.y);
        }

        bool collision(Player p) {
            // distance collision detection
            if (sqrt(pow(this->bodyCenter.getPosition().x - p.bodyCenter.getPosition().x, 2) +
                     pow(this->bodyCenter.getPosition().y - p.bodyCenter.getPosition().y, 2)) >=
                     this->bodyCenter.getRadius() + p.bodyCenter.getRadius()) {
                return false;
            }
            return true;
        }
